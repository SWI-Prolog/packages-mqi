...
use swipl_rs::{PrologCompound, PrologError, PrologTerm, QueryResult, ServerConfig};
use test_helpers::*;
...
fn test_async_query() {
    ...
    session
        .query_async("member(X, [a, b, c])", false, None)
        ...
        .expect("Failed to get result")
    {
        ...
        match result {
            QueryResult::Solutions(solutions) => {
                ...
                assert_eq!(
                    solutions[0].get("X"),
                    Some(&PrologTerm::Atom(expected_values[count].to_string()))
                );
                count += 1;
            }
...
fn test_cancel_async_query() {
    ...
    let handle = thread::spawn(move || {
        // Start the query
        session
            .query_async("repeat, fail", false, None)
            .expect("Failed to start async query");
        ...
        match result {
            ...
            _ => panic!(
                "Expected no results, NoQuery error, or QueryCancelled, got {:?}",
                result
            ),
        }
    ...
    sync.wait_ready(Duration::from_secs(5))
        .expect("Query didn't start in time");
...
fn test_syntax_error() {
    ...
    match result {
        Err(PrologError::PrologException { kind, .. }) => {
            assert!(
                kind.contains("syntax_error") || kind.contains("error"),
...
fn test_multiple_sessions() {
    ...
    let result1 = session1
        .query("X = session1", None)
        .expect("Query 1 failed");
    let result2 = session2
        .query("Y = session2", None)
    ...
    match result1 {
        QueryResult::Solutions(solutions) => {
            ...
            assert_eq!(
                solutions[0].get("X"),
                Some(&PrologTerm::Atom("session1".to_string()))
            );
        }
    ...
    match result2 {
        QueryResult::Solutions(solutions) => {
            ...
            assert_eq!(
                solutions[0].get("Y"),
                Some(&PrologTerm::Atom("session2".to_string()))
            );
        }
...
fn test_query_with_atoms_and_strings() {
    ...
    let result = session
        .query("A = hello, B = 'Hello World', C = \"string\"", None)
        .expect("Query failed");
    assert_has_solutions(&result, Some(1));
    ...
    match result {
        QueryResult::Solutions(solutions) => {
            ...
            assert_eq!(
                solution.get("B"),
                Some(&PrologTerm::Atom("Hello World".to_string()))
            );
            assert_eq!(
                solution.get("C"),
                Some(&PrologTerm::Atom("string".to_string()))
            );
        }
        _ => unreachable!(),
    }
...
fn test_multiple_solutions() {
    ...
    let result = session
        .query("(X = 1, Y = a); (X = 2, Y = b); (X = 3, Y = c)", None)
        .expect("Query failed");
    assert_has_solutions(&result, Some(3));
    ...
    match result {
        QueryResult::Solutions(solutions) => {
            ...
            assert_eq!(
                solutions[0].get("Y"),
                Some(&PrologTerm::Atom("a".to_string()))
            );
            assert_eq!(solutions[1].get("X"), Some(&PrologTerm::Integer(2)));
            assert_eq!(
                solutions[1].get("Y"),
                Some(&PrologTerm::Atom("b".to_string()))
            );
            assert_eq!(solutions[2].get("X"), Some(&PrologTerm::Integer(3)));
            assert_eq!(
                solutions[2].get("Y"),
                Some(&PrologTerm::Atom("c".to_string()))
            );
        }
        _ => unreachable!(),
    }
...
fn test_prolog_assert_and_retract() {
    ...
    // Assert a fact
    let result = session
        .query("assert(test_fact(hello))", None)
    ...
    // Retract the fact
    let result = session
        .query("retract(test_fact(_))", None)
