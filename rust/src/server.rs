...
use crate::session::{ConnectionAddr, PrologSession};
...
impl PrologServer {
    ...
    pub fn new(config: ServerConfig) -> Result<Self, PrologError> {
        ...
        if config.unix_domain_socket.is_some() {
            ...
            return Err(PrologError::FeatureNotEnabled(
                "Unix Domain Sockets require the 'unix-socket' feature on Unix-like systems"
                    .to_string(),
            ));
            #[cfg(all(unix, feature = "unix-socket"))]
    ...
    pub fn start(&mut self) -> Result<(), PrologError> {
        ...
        let prolog_executable = self.config.prolog_path.as_ref().ok_or_else(|| {
            PrologError::ConfigError(
                "Path to SWI-Prolog executable (prolog_path) not configured".to_string(),
            )
        })?;
        ...
        command.arg(format!(
            "--password={}",
            self.effective_password.as_ref().unwrap()
        ));
            ...
            #[cfg(all(unix, feature = "unix-socket"))]
            {
                ...
                if _uds_path_config.as_os_str().is_empty() {
                    ...
                    if full_socket_path.as_os_str().len() > UNIX_SOCKET_PATH_MAX_LEN {
                        ...
                        return Err(PrologError::InvalidState(
                            "Generated UDS path is too long".to_string(),
                        ));
                    }
                    ...
                    debug!(
                        "Generated UDS path: {:?}",
                        self.effective_uds_path.as_ref().unwrap()
                    );
                } else {
                    // Use provided path
                    ...
                    command.arg(format!(
                        "--unix_domain_socket={}",
                        create_prolog_path(_uds_path_config)?
                    ));
                }
            }
            #[cfg(not(all(unix, feature = "unix-socket")))]
            return Err(PrologError::FeatureNotEnabled(
                "unix-socket feature required".into(),
            ));
        } else {
            // Using TCP
        ...
        // --- Spawn Process ---
        let args_for_debug: Vec<String> = command
            .get_args()
        ...
        let child_stdout = child.stdout.take().ok_or_else(|| {
            PrologError::LaunchError("Failed to capture swipl stdout".to_string())
        })?;
        ...
        if self.effective_uds_path.is_some() {
            ...
            if self.effective_uds_path.as_ref().unwrap().to_str() != Some(conn_detail.as_str())
                && create_uds
            {
                warn!(
                    "Generated UDS path mismatch: expected {:?}, got {}",
                    self.effective_uds_path.as_ref().unwrap(),
                    conn_detail
                );
            ...
            } else if self.effective_uds_path.as_ref().unwrap().to_str()
                != Some(conn_detail.as_str())
            {
                return Err(PrologError::LaunchError(format!(
                    "UDS path mismatch: expected {:?}, got {}",
                    self.effective_uds_path.as_ref().unwrap(),
                    conn_detail
                )));
            }
            ...
            let port: u16 = conn_detail.parse().map_err(|_| {
                PrologError::LaunchError(format!(
                    "Failed to parse port number from swipl stdout: {}",
                    conn_detail
                ))
            })?;
        ...
        if let Some(expected_password) = self.effective_password.as_ref() {
            if *expected_password != password_from_prolog {
                ...
                return Err(PrologError::LaunchError(format!(
                    ...
                    expected_password, password_from_prolog
                )));
            }
        ...
        let _stdout_reader_thread = thread::Builder::new()
            .name(format!("swipl-{}-stdout", process_id))
            .spawn(move || {
            ...
            .map_err(|e| {
                PrologError::LaunchError(format!("Failed to spawn stdout reader thread: {}", e))
            })?;
        ...
        if let Some(traces) = traces_to_set {
    ...
    pub fn connect(&mut self) -> Result<PrologSession, PrologError> {
        ...
        let _password = self.effective_password.clone().ok_or_else(|| {
            PrologError::InvalidState("Password not available for connection".to_string())
        })?;
        ...
        let address = self
            ...
            .or_else(|| {
                self.effective_port
                    .map(|p| Ok(ConnectionAddr::Tcp(self.effective_host.clone(), p)))
            })
            .ok_or_else(|| {
                PrologError::InvalidState(
                    "No valid connection address (port/UDS) available".to_string(),
                )
            })??;
        ...
        if let Some(mut child) = self.process.take() {
            ...
            if !kill && !conn_failed {
                ...
                match self.connect() {
                    ...
                    Err(e) => {
                        warn!(
                            "Failed to connect for graceful shutdown of PID {}: {}. Will kill.",
                            pid, e
                        );
                    }
                }
            ...
            match child.kill() {
                ...
                Err(e) => {
                    error!(
                        "Failed to send kill signal to SWI-Prolog process PID {}: {}",
                        pid, e
                    );
            ...
            match child.wait() {
                Ok(status) => info!(
                    "SWI-Prolog process PID {} exited with status: {}",
                    pid, status
                ),
                Err(e) => error!(
                    "Failed to wait for SWI-Prolog process PID {} to exit: {}",
                    pid, e
                ),
            }
            result = Ok(());
        ...
        if let Some(_dir_path) = uds_dir_to_clean {
            #[cfg(all(unix, feature = "unix-socket"))]
            {
                ...
                if let Err(e) = fs::remove_dir_all(&_dir_path) {
                    warn!(
                        "Failed to remove generated UDS directory {:?}: {}",
                        _dir_path, e
                    );
                }
            }
...
impl Drop for PrologServer {
    fn drop(&mut self) {
        if self.process.is_some() {
            warn!(
                "PrologServer dropped without explicit stop(), killing process PID {}.",
        ...
        if let Some(_dir_path) = self.generated_uds_dir.take() {
            ...
            {
                warn!(
                    "Cleaning up generated UDS directory {:?} during drop",
                    _dir_path
                );
                if let Err(e) = fs::remove_dir_all(&_dir_path) {
                    error!(
                        "Failed to remove generated UDS directory {:?} during drop: {}",
                        _dir_path, e
                    );
                }
            }
...
fn create_prolog_path(path: &PathBuf) -> Result<String, PrologError> {
    ...
    path.to_str().map(|s| s.to_string()).ok_or_else(|| {
        PrologError::InvalidState(format!("Path contains invalid UTF-8: {:?}", path))
    })
}
